...
const submit = async (value) => {
  ...
  axiosInstance
    ...
    .then(({ data }) => {
      ...
      if (AttachmenmtForm.value.File != '') {
        Swal.fire({
          title:
            'Operation accomplished successfully, Wait for the attachment to be uploaded',
          icon: 'success',
        ...
        uploadAttachmentWithRquest(data.data.id);
        return;
      }
      Swal.fire({
...
const uploadAttachmentWithRquest = async (value) => {
  isLoading.value = true;
  isError.value = false;
  ...
  axiosInstance
    .post('Main/AddAttachment?requestId=' + value, formData)
    .then(({ data }) => {
      isLoading.value = false;
      isInfoModalOpen.value = false;
      Swal.fire({
        ...
        getRequest();
      AttachmenmtForm.value.File = '';
      AttachmenmtForm.value.id = '';
    })
    .catch((error) => {
      isLoading.value = false;
      Swal.fire({
    ...
    axiosInstance
      .get(
        `Main/GetRequset?userRequest=${isUser}&PageNumber=${
          pageNumber.value
        }&PageSize=${pageSize.value}&date=${
          searchform.value.date
        }&companyName=${searchform.value.companyName}&requestNumber=${
          searchform.value.requestNumber
        }&status=${searchform.value.status}&fullName=${
          searchform.value.fullName
        }&isAll=${props.isReport ? 'true' : 'false'}`
      )
      .then(({ data }) => {
                ...
                <td class="xl:py-3 xl:px-6 py-2 px-4">
                  <div class="flex items-center gap-2">
                    ...
                    <PhFileText
                      ...
                      "
                      class="w-6 h-6 fill-slate-900 hover:scale-105 transition-all duration-300 cursor-pointer" />
                  </div>
                </td>
              </tr>
            </tbody>
