...
const getRequest = () => {
  ...
  axiosInstance
    .get(
      `Main/GetRequset?userRequest=${isUser}&PageNumber=${pageNumber.value}&PageSize=${pageSize.value}&date=${searchform.value.date}&companyName=${searchform.value.companyName}&requestNumber=${searchform.value.requestNumber}&status=Finished&fullName=${searchform.value.fullName}&isAll=true`
    )
    .then(({ data }) => {
...
<template>
  ...
  <MainModal styles="max-w-max" text="Add a new request" v-if="isAddModalOpen" @close="isAddModalOpen = false">
    <Form class="gap-4" :validationSchema="validationSchema" @submit="submit">
      <!-- Title -->
